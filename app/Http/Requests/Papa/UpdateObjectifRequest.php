...
class UpdateObjectifRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        if ($objectif instanceof \App\Models\Objectif) {
            return $this->user()->can('update', $objectif);
        }
        return $this->user()->can('update', \App\Models\Objectif::class);
    }
    ...
    {
        $objectif = $this->route('objectif');
        $objectifId = $objectif instanceof \App\Models\Objectif ? $objectif->id : $objectif;
        
        return [
            'papa_version_id' => ['required', 'exists:papa_versions,id'],
            'code' => ['required', 'string', 'max:32', Rule::unique('objectifs', 'code')->ignore($objectifId)],
