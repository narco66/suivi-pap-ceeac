<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="mb-0 fw-bold text-ceeac-blue">
                <i class="bi bi-pencil me-2"></i>Modifier la commission
            </h2>
            <a href="{{ route('commissions.show', $commission) }}" class="btn btn-outline-secondary">
    ...
    <div class="card card-ceeac">
        <div class="card-header bg-ceeac-gradient text-white">
            <h5 class="mb-0">
                <i class="bi bi-people me-2"></i>Informations de la commission
            </h5>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('commissions.update', $commission) }}" id="commissionForm">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-4">
                    <div class="col-md-4">
                        ...
                        <input 
                            ...
                            id="code" 
                            name="code" 
                            value="{{ old('code', $commission->code) }}" 
                        ...
                        @enderror
                        <small class="form-text text-muted">Code unique de la commission.</small>
                    </div>
                    <div class="col-md-8">
                        ...
                        <input 
                            ...
                            id="libelle" 
                            name="libelle" 
                            value="{{ old('libelle', $commission->libelle) }}" 
                        ...
                        @error('libelle')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="form-text text-muted">Titre descriptif de la commission.</small>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-md-12">
                        ...
                        <textarea 
                            ...
                            name="description" 
                            rows="4"
                            placeholder="Description détaillée de la commission..."
                        >{{ old('description', $commission->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="form-text text-muted">Description complète de la commission et de ses missions.</small>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        ...
                        >
                            <option value="1" {{ old('actif', $commission->actif ? '1' : '0') == '1' ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ old('actif', $commission->actif ? '1' : '0') == '0' ? 'selected' : '' }}>Inactive</option>
                        </select>
                        @error('actif')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="form-text text-muted">Statut de la commission dans le système.</small>
                    </div>
                </div>
