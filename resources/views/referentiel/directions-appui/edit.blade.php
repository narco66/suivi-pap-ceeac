<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="mb-0 fw-bold text-ceeac-blue">
                <i class="bi bi-pencil me-2"></i>Modifier la direction d'appui
            </h2>
            <a href="{{ route('directions-appui.show', $directionAppui) }}" class="btn btn-outline-secondary">
    ...
    <div class="card card-ceeac">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('directions-appui.update', $directionAppui) }}" id="directionAppuiForm">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-4">
                    <div class="col-md-4">
                        ...
                        <input 
                            ...
                            id="code" 
                            name="code" 
                            value="{{ old('code', $directionAppui->code) }}" 
                        ...
                        @enderror
                        <small class="form-text text-muted">Code unique identifiant la direction d'appui.</small>
                    </div>
                    <div class="col-md-8">
                        ...
                        <input 
                            ...
                            id="libelle" 
                            name="libelle" 
                            value="{{ old('libelle', $directionAppui->libelle) }}" 
                        ...
                        @error('libelle')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="form-text text-muted">Nom complet de la direction d'appui.</small>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        ...
                        >
                            <option value="1" {{ old('actif', $directionAppui->actif ? '1' : '0') == '1' ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ old('actif', $directionAppui->actif ? '1' : '0') == '0' ? 'selected' : '' }}>Inactive</option>
                        </select>
                        @error('actif')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="form-text text-muted">Statut de la direction d'appui dans le système.</small>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-12">
                        ...
                        <textarea 
                            ...
                            name="description" 
                            rows="4"
                            placeholder="Description détaillée de la direction d'appui..."
                        >{{ old('description', $directionAppui->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="form-text text-muted">Description détaillée des missions et responsabilités de la direction d'appui.</small>
                    </div>
                </div>
