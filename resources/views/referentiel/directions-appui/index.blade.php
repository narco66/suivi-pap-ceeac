<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            ...
            @can('create', \App\Models\DirectionAppui::class)
                <a href="{{ route('directions-appui.create') }}" class="btn btn-primary btn-ceeac">
                    <i class="bi bi-plus-circle me-2"></i>Créer une direction d'appui
                </a>
            @endcan
        </div>
    </x-slot>
    ...
    <div class="card card-ceeac mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('directions-appui.index') }}" class="row g-3">
                <div class="col-md-6">
    ...
    <!-- Liste des directions d'appui -->
    <div class="card card-ceeac">
        <div class="card-header bg-white">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="mb-0">
                    <i class="bi bi-list-ul me-2"></i>Liste des directions d'appui
                    @if(isset($directionsAppui) && $directionsAppui->count() > 0)
                        <span class="badge bg-primary ms-2">{{ $directionsAppui->total() }}</span>
                    @endif
                </h5>
                @can('create', \App\Models\DirectionAppui::class)
                    <a href="{{ route('directions-appui.create') }}" class="btn btn-primary btn-ceeac">
            ...
            @if(isset($directionsAppui) && $directionsAppui->count() > 0)
                <div class="table-responsive">
                    <table class="table table-ceeac table-hover">
                        ...
                        <tbody>
                            @foreach($directionsAppui as $directionAppui)
                                <tr>
                                    <td>
                                        <strong>{{ $directionAppui->code }}</strong>
                                    </td>
                                    <td>
                                        <strong>{{ $directionAppui->libelle }}</strong>
                                    </td>
                                    <td>
                                        @if($directionAppui->description)
                                            {{ \Illuminate\Support\Str::limit($directionAppui->description, 50) }}
                                        @else
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            @can('view', $directionAppui)
                                                <a href="{{ route('directions-appui.show', $directionAppui) }}" class="btn btn-sm btn-outline-primary" title="Voir">
                                                    <i class="bi bi-eye"></i>
                                                </a>
                                            @endcan
                                            @can('update', $directionAppui)
                                                <a href="{{ route('directions-appui.edit', $directionAppui) }}" class="btn btn-sm btn-outline-secondary" title="Modifier">
                ...
                @if(method_exists($directionsAppui, 'hasPages') && $directionsAppui->hasPages())
                    {{ $directionsAppui->links() }}
                @endif
            @else
                <div class="text-center text-muted py-5">
                    <i class="bi bi-inbox fs-1 d-block mb-3"></i>
                    <p class="mb-0">Aucune direction d'appui enregistrée</p>
                    @if(request()->hasAny(['search', 'actif']))
                        <a href="{{ route('directions-appui.index') }}" class="btn btn-outline-secondary mt-3">
