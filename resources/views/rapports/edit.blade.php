<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="mb-0 fw-bold text-ceeac-blue">
                <i class="bi bi-pencil me-2"></i>Modifier le rapport
            </h2>
            <a href="{{ route('rapports.show', $rapport) }}" class="btn btn-outline-secondary">
    ...
    <div class="card card-ceeac">
        ...
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('rapports.update', $rapport) }}" id="rapportForm">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3 mb-4">
                    <div class="col-12">
                        ...
                        <textarea 
                            ...
                            name="description" 
                            rows="3"
                            placeholder="Description du rapport...">{{ old('description', $rapport->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12">
                        <div class="alert alert-info">
                            ...
                            @if($rapport->scope_level === 'GLOBAL')
                                <span class="badge bg-primary">Global (Institutionnel)</span>
                            @elseif($rapport->scope_level === 'SG')
                                <span class="badge bg-success">Secrétaire Général (Directions d'Appui)</span>
                            @elseif($rapport->scope_level === 'COMMISSAIRE')
                                <span class="badge bg-warning">Commissaire (Département Technique)</span>
                            @endif
                            <small class="d-block mt-2">Le périmètre ne peut pas être modifié après la création du rapport.</small>
                        </div>
                    </div>
                </div>
                @endif
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        ...
                        >
                            ...
                            <option value="papa" {{ old('type', $rapport->type) === 'papa' ? 'selected' : '' }}>PAPA</option>
                            <option value="objectif" {{ old('type', $rapport->type) === 'objectif' ? 'selected' : '' }}>Objectif</option>
                            <option value="action_prioritaire" {{ old('type', $rapport->type) === 'action_prioritaire' ? 'selected' : '' }}>Action Prioritaire</option>
                            <option value="tache" {{ old('type', $rapport->type) === 'tache' ? 'selected' : '' }}>Tâche</option>
                            <option value="kpi" {{ old('type', $rapport->type) === 'kpi' ? 'selected' : '' }}>KPI</option>
                            <option value="avancement" {{ old('type', $rapport->type) === 'avancement' ? 'selected' : '' }}>Avancement</option>
                            <option value="alerte" {{ old('type', $rapport->type) === 'alerte' ? 'selected' : '' }}>Alerte</option>
                            <option value="synthese" {{ old('type', $rapport->type) === 'synthese' ? 'selected' : '' }}>Synthèse</option>
                            <option value="risques_retards" {{ old('type', $rapport->type) === 'risques_retards' ? 'selected' : '' }}>Risques & Retards</option>
                            <option value="personnalise" {{ old('type', $rapport->type) === 'personnalise' ? 'selected' : '' }}>Personnalisé</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <select 
                            ...
                            required
                        >
                            <option value="pdf" {{ old('format', $rapport->format) === 'pdf' ? 'selected' : '' }}>PDF</option>
                            <option value="excel" {{ old('format', $rapport->format) === 'excel' ? 'selected' : '' }}>Excel</option>
                            <option value="csv" {{ old('format', $rapport->format) === 'csv' ? 'selected' : '' }}>CSV</option>
                            <option value="html" {{ old('format', $rapport->format) === 'html' ? 'selected' : '' }}>HTML</option>
                        </select>
                        @error('format')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-4">
                        ...
                        <select 
                            ...
                            required
                        >
                            <option value="jour" {{ old('periode', $rapport->periode) === 'jour' ? 'selected' : '' }}>Jour</option>
                            <option value="semaine" {{ old('periode', $rapport->periode) === 'semaine' ? 'selected' : '' }}>Semaine</option>
                            <option value="mois" {{ old('periode', $rapport->periode) === 'mois' ? 'selected' : '' }}>Mois</option>
                            <option value="trimestre" {{ old('periode', $rapport->periode) === 'trimestre' ? 'selected' : '' }}>Trimestre</option>
                            <option value="semestre" {{ old('periode', $rapport->periode) === 'semestre' ? 'selected' : '' }}>Semestre</option>
                            <option value="annee" {{ old('periode', $rapport->periode) === 'annee' ? 'selected' : '' }}>Année</option>
                            <option value="personnalise" {{ old('periode', $rapport->periode) === 'personnalise' ? 'selected' : '' }}>Personnalisé</option>
                        </select>
                        @error('periode')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-4" id="date_debut_field" style="{{ old('periode', $rapport->periode) === 'personnalise' ? '' : 'display: none;' }}">
                        ...
                        <input 
                            ...
                            id="date_debut" 
                            name="date_debut" 
                            value="{{ old('date_debut', $rapport->date_debut?->format('Y-m-d')) }}"
                        >
                        @error('date_debut')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-4" id="date_fin_field" style="{{ old('periode', $rapport->periode) === 'personnalise' ? '' : 'display: none;' }}">
                        ...
                        <input 
                            ...
                            id="date_fin" 
                            name="date_fin" 
                            value="{{ old('date_fin', $rapport->date_fin?->format('Y-m-d')) }}"
                        >
                        @error('date_fin')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        >
                            ...
                            @foreach($papas ?? [] as $papa)
                                <option value="{{ $papa->id }}" {{ old('papa_id', $rapport->papa_id) == $papa->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        >
                            ...
                            @foreach($objectifs ?? [] as $objectif)
                                <option value="{{ $objectif['id'] }}" {{ old('objectif_id', $rapport->objectif_id) == $objectif['id'] ? 'selected' : '' }}>
                                    {{ $objectif['libelle'] }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-12">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="est_automatique" name="est_automatique" value="1" {{ old('est_automatique', $rapport->est_automatique) ? 'checked' : '' }}>
                            <label class="form-check-label fw-semibold" for="est_automatique">
                                <i class="bi bi-clock-history me-1"></i>Génération automatique
                            </label>
                        </div>
                    </div>
                    <div class="col-md-6" id="frequence_cron_field" style="{{ old('est_automatique', $rapport->est_automatique) ? '' : 'display: none;' }}">
                        ...
                        >
                            ...
                            <option value="daily" {{ old('frequence_cron', $rapport->frequence_cron) === 'daily' ? 'selected' : '' }}>Quotidien</option>
                            <option value="weekly" {{ old('frequence_cron', $rapport->frequence_cron) === 'weekly' ? 'selected' : '' }}>Hebdomadaire</option>
                            <option value="monthly" {{ old('frequence_cron', $rapport->frequence_cron) === 'monthly' ? 'selected' : '' }}>Mensuel</option>
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input 
                            ...
                            id="destinataires" 
                            name="destinataires" 
                            value="{{ old('destinataires', $rapport->destinataires) }}" 
                ...
                <div class="row g-3 mb-4">
                    <div class="col-12">
                        ...
                        <textarea 
                            ...
                            name="notes" 
                            rows="2"
                            placeholder="Notes internes sur ce rapport...">{{ old('notes', $rapport->notes) }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
